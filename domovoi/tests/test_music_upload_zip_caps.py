...
def _request() -> Request:
    """A minimal ASGI-scope Request. The endpoint takes one so the web hop
...
async def test_member_over_the_per_file_cap_returns_413_before_extraction(
    ...
    with pytest.raises(HTTPException) as exc:
        await music_api.upload_to_library(_request(), files=[_upload("album.zip", payload)])
...
async def test_members_over_the_total_cap_return_413(music_dir, reindex_calls) -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        await music_api.upload_to_library(_request(), files=[_upload("box.zip", bytes(raw))])
...
async def test_too_many_members_still_returns_413(music_dir, reindex_calls) -> None:
    ...
    with pytest.raises(HTTPException) as exc:
        await music_api.upload_to_library(_request(), files=[_upload("many.zip", buf.getvalue())])
    assert exc.value.status_code == 413
