...
flowchart TB
    ...
    subgraph device["Device tier — X-Device-Token (or admin Bearer)"]
        ...
        v2["Subscribe to a podcast, poll feeds now,<br/>attach or re-test a news feed"]
    end
...
before setup. The routes that make the server go and fetch something a
caller chose wear it first (podcast subscribe and poll, news feed attach
and re-test); the rest of the ordinary routes (music, queue, announce,
intent…) move onto it next.
