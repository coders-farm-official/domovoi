...
roughly 3k tokens and grows with every plugin, so a router that starts
missing commands as plugins are added may be running out of room.
`ollama_tool_num_ctx` (the router) and `ollama_num_ctx` (Q&A and the
dashboard's text chat) set it per role, under dashboard → **Models**;
8192 is a sensible first step. A bigger window costs memory for as long as
the model stays loaded, and Ollama reloads a model whenever its window
changes, so if one model serves both roles, give both the same value.
