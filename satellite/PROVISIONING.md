...
- [ ] **Pi Zero 2 W / older Pis — force full-speed USB.** The XVF3800 enumerates at USB high speed if allowed, and at high speed on the Pi's controller its capture stream is **not audio**: `mic_probe.py` reads `frames/sec ≈ 128000` (~8× the 16 kHz asked for), and the samples are junk — measured at 13 distinct values per 480-sample block, held in runs of eight. No resampling recovers it. The symptom is relentless `mic queue overflowing` and a wake word that never fires. Fix: add `dwc_otg.speed=1` to the **single line** of `/boot/firmware/cmdline.txt` (do not add a newline — a malformed cmdline can stop the Pi booting), then `sudo reboot`. Cards from media prep get this token at prepare time; USB-adopted units get it when adoption reverts gadget mode. Host mode itself comes from the OTG adapter's ID pin under the Pi's default `dwc_otg` driver. **Do NOT add `dtoverlay=dwc2,dr_mode=host`.** It was documented here for a while as a way to run a plain data cable without an OTG adapter, and as "clocking the audio correctly". It does neither reliably: it replaces `dwc_otg`, which makes `dwc_otg.speed=1` inert, and it enumerates the array at high speed — the broken mode. It worked exactly once, on one lucky full-speed enumeration, and both boards we own fail with it. Verify: `mic_probe.py` should read `frames/sec ≈ 16000`, the client log should show `usb device … speed=12 Mbit/s`, and there should be no `mic delivers … the negotiated rate` error at startup.
