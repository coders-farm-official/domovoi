...
- [ ] Pin the music ALSA device to the array by name (don't rely on the `"default"` fallback):
  ...
  ```
- [ ] **Remove or comment out any HAT-specific overrides** you'd set previously, so the XVF profile's defaults take over. In particular, if your config has explicit `[mic_gain] enabled = true`, `[noise_gate] dbfs = -45`, `[barge_in] vad_aggressiveness_during_tts = 3`, or `[leds] num_leds = 3 / brightness = 16`, comment them out. (If you never set them — they were profile defaults — there's nothing to remove.) `[noise_gate] auto_calibrate = true` is the default on every board now, so it can stay.
  - `[barge_in] require_wake_word = true` is also the default on every board now (it used to be a HAT-era workaround for the HAT's speaker leaking into the mic). The XVF3800's on-chip AEC removes that echo, which makes this the one board where you can safely set it to `false` and interrupt Domovoi by **just talking over him** instead of saying "hey jarvis stop". That is an opt-in: barge-in itself ships off, so turn on `[barge_in] enabled` first.
