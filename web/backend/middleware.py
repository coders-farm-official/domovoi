...
from domovoi.transport_guard import lan_origin_regex
...
def cors_origin_regex(port: int) -> str:
    ...
    The host alternatives themselves come from
    :func:`domovoi.transport_guard.lan_origin_regex` — ONE source shared
    with the core's WebSocket ``Origin`` check (CORE-8), so a page that
    cannot call the REST API cannot open a socket either, and a household
    that reaches Domovoi through a name in ``TRUSTED_HOSTS`` or on a
    ``.lan`` / ``.home.arpa`` / ``.internal`` suffix is not locked out of
    one surface while the other works. This function only pins the port.
    """
    return lan_origin_regex(int(port))
